...
async function deployToOptimism() {
    ...
    const hashes = await Promise.all(
        [SimpleDestinationSettler, HelloWorld].map((contract, idx) =>
...
async function deployToBase(destinationSettlerAddress, crossL2Address) {
    if (!destinationSettlerAddress || !crossL2Address) {
        throw new Error("DestinationSettler and CrossL2Address can't be empty")
    }
    ...
    const hash = await baseClient.deployContract({
        ...
        nonce,
        args: [destinationSettlerAddress, crossL2Address],
    })
...
async function deploy() {
    try {
        ...
        const optimismReceipts = await deployToOptimism()

        console.log('args:', optimismReceipts[0].contractAddress, process.env.CROSS_L2_ADDRESS)
        const baseReceipts = await deployToBase(optimismReceipts[0].contractAddress, process.env.CROSS_L2_ADDRESS)
        ...
        console.log('- SimpleOriginSettler:', baseReceipts.contractAddress)
    } catch (error) {
